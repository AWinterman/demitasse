...
proto.on = function(name, callback, response_headers){
  ...
  this.stream.on(
      name
    , framework_for_response.bind(this, callback, response_headers)
  )
  return this
}
...
function framework_for_response(callback, response_headers, embedded_args){
  var response_headers = response_headers || function(x){ return x }
  this.stream.emit("headers", wrapped_response_headers)
  this.stream.emit("data", wrapped_data)
