...
// Initialize Demi with an array of routes, and an optional `function(err,
// data)` which is called whenever a static file is successfully served

function Demi(static_root, static_callback){
  this.file = new (static.Server)(static_root)
  this.routes = []
  this.static_callback = static_callback
}
...
proto.run = function(port){
  ...
  function demi_server(req, resp) {
    ...
    if (!responder[0]) {
      ...
      responder[0].stream.on("data", function(callback){ callback(req, resp) })
    }
    // may eventually want to allow cleanup on the end event, yet a third
    // argument to the .on method
    responder[0].stream.emit(req.method, req, resp)
  }
